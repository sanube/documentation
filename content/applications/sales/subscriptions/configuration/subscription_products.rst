...
The *Subscriptions* app comes with some basic recurrence periods already configured. These can be
edited as needed, and any number of new recurrence periods can be added.
...
.. example::
   To create a recurrence period such that a subscription renews every two weeks, set the
...
In the :guilabel:`Period` column, select a recurrence period (e.g. Weekly, Monthly, Yearly). In the
:guilabel:`Price` column, type in the price for that recurrence period.

To create pricing rules for specific pricelists, select a pricelist in the :guilabel:`Pricelist`
column. This allows customers belonging to this pricelist to receive special recurring pricing for
...
.. note::
   ...
   :guilabel:`Recurring` on the product form and configuring :guilabel:`Time-based pricing`.

   A product can be both a regular sales product and a subscription product at the same time.
